...
def normalize_layer_key(result: str) -> str:
    """Normalize layer name by stripping prefixes and suffixes."""
    layer_key = result
    prefix = "Wassertiefe_"
...
async def send_kml_if_available(update: Update, layer_key: str) -> None:
    """Send the KML file for a given layer if it exists."""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if not result:
            await update.message.reply_text("⚠️ Keine Auswertung möglich.")
        ...
        await update.message.reply_text(f"📍 Gefundener Layer: {result}")
        logger.info(f"Analyse erfolgreich: {result}")
        await send_kml_if_available(update, layer_key)
